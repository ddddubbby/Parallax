...
xAI provider adapter (new); OpenAI + Gemini grounded paths; citation normalization behind the C-10 gate (empty/malformed citations fail the call); the three-token live spike (VIRTUAL/Base, TOSHI/Base, PEPE/Ethereum); pin cheapest-verified model IDs, measured per-job COGS, p99 latency, and Lane-B matched rates; freeze `crypto_geo_prompts_v1`. **Merge when:** all three live engines produce 100 technically complete samples each; a sparse token renders honest `representation_state`; measured cost fits the §3 envelope; all pins evidenced in BUILD_NOTES.

### 6.5 M39 — Commerce persistence + effectively-once effects (offline, $0)
The §4.3 migration and repositories; the order state machine and effects ledger (§4.5) driven entirely by simulated/replayed event fixtures — no network; gateway leadership + per-order advisory locks. **Merge when:** the chaos suite shows ZERO duplicate external effects under duplicate/out-of-order/dropped events, process crashes at every effect boundary, old/new instance overlap, DB outage, and ambiguous transaction responses.
...
Render topology (§5.2), pool caps, graceful drains, liveness/readiness, observability (§5.3), all §5.4 runbooks. Deploy with `ACP_FORCE_DISABLED=true` → migrations → reconcile-only → verify provider/contract/registry/wallet health → enable hidden sandbox admissions. **Merge when:** a 24-hour soak shows no heartbeat gap, memory growth, state mismatch, settlement discrepancy, or digest failure; chaos tests recover within SLO.

### 6.8 M42 — Graduation and production (~$700 cycled through graduation tests)
≥ 10 successful $0.01 sandbox jobs incl. 3 consecutive from the separate test buyer; the failure-case sweep (invalid/missing/conflicting input, unsupported evaluator/hook, funding timeout, late funding, provider outage, funded rejection, expiry, restart, report fetch); freeze code/schema/description/samples/methodology/model catalog; switch the still-hidden offering to $99; one controlled $99 auto-approval job freezing the observed fee/settlement catalog; DevRel evaluator tests (allowlist only its exact wallet); 100% automated evaluation; submission package; stay online through the ~5–10-business-day manual review; after Shown: remove the allowlist, verify zero-evaluator production policy, one paid canary, launch concurrency 1. **Merge when:** the paid canary is settled and reconciled; STATUS.md flips the product to operating. All graduation numbers re-verified against live Virtuals docs at M42 start.
