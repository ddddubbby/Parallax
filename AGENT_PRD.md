...
- **Redaction is a versioned deterministic policy (`redact_v1`), not a judgment call.** Redact only: exposed credentials/secrets, sensitive personal data, content illegal to redistribute. Every redaction retains source hash, offsets, policy code, and omission reason. Raw evidence stays immutable and access-controlled internally.
