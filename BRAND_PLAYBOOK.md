...
Synthetic evidence is the easiest evidence in the world to fake. That is
...
## 5. Claims law (the honesty rail, externalized)

This section is enforceable brand QA. Treat it like a test suite: any new
external sentence with a number in it must pass all five rules.
...
4. **Labels travel with numbers.** Directional stays directional in the deck,
   ...
   are described as exactly that (5.2).
5. **Real receipts only.** Any stat on any surface must trace to a stored
   run, the independently published paper (attributed), or a citable third party. If a
   number cannot be substantiated, it is cut, not rounded.

### 5.2 The calibration fix (deck errata, binding)

The current deck's "calibrated preference construct" is wrong and must be
replaced. Approved language: "**scored against a benchmarked intent
construct**, using anchor sets from the independently published method; calibration against
matched human panels is on the roadmap and results ship labeled until then."
Short form for slides: "benchmarked, not black-boxed."

### 5.3 Banned vocabulary (grep list for any external asset)

"guaranteed" (any ranking context) · "#1 in ChatGPT/AI" · "ROI" as a
simulation output · "purchase probability" · "conversion lift" · "calibrated"
(until a calibration study exists) · "Resonance Tensor" · "Perceptual
Manifold" · "Latent Preference Triangulation" · "Signal Fidelity Index" ·
"proprietary algorithm" (we are proudly method-transparent) · the internal
engine codename · "real-time" (runs take minutes to hours; say "in hours") ·
"AI-powered" as a differentiator (everyone says it; say what the system does) ·
"our research" / "our validation" / "we published" (the SSR paper is
independent; we productize it).

### 5.4 Approved claim inventory (v1)
...
| "Every audit prompt is sampled five times per model and mode (k=5); a Message Lift test uses one AI model at k=5." | D-119 / PRD §8 | Product fact |
...
Panels are conditioned on **age and income** (the axes validated in the independent
research we build on) with location and behavioral context as descriptive color. Never
claim validated segmentation beyond age and income; never condition on gender
or ethnicity.
...
**We are an instrument, not a hype machine.** Calm declaratives. Short
sentences. A number beside every adjective. Confidence from precision, not
volume. We never punch down at research professionals; we retired their
constraint (time and cost), not their craft.

| Instead of | Write |
|---|---|
| "Revolutionary AI-powered consumer insights at unprecedented speed!" | "Twenty agents before lunch. Scored with an independently published method. Labeled honestly." |
| "Know exactly what your customers think." | "Test which framing moves expressed intent, before you spend." |
| "Our proprietary AI simulates your customers with 90% accuracy." | "SSR, from independent peer-reviewed research, reaches 90% of human test-retest reliability. We run it on framings measured from the AI channel, and label everything we cannot yet prove." |
| "Never run an expensive survey again." | "Save the human panel for the decisions that need one. Close the loop on everything else." |
...
- New external claims enter section 5.4's table or they do not ship.
- Any external asset (deck, page, PDF) gets a pre-flight grep against 5.3's
  banned list before it leaves the building.
- The playbook is versioned in git; material positioning changes get a dated
  entry appended below.

**v1 — 2026-07-07.** Initial playbook. Decisions: category = "synthetic
consumer research grounded in evidence"; simulation is the hero, audit is the
evidence layer; engine codename retired externally; page-7 jargon killed;
"calibrated" corrected to benchmarked-with-roadmap; pilot anonymized to "a
leading Asian tea brand," framed as self-initiated; the Glass Box coined as
the honesty asset.
