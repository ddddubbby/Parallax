...
- M36–M40 (GEO agent): retired and removed in M62 (D-141); their acceptance commands no longer exist. See `docs/history/AGENT_BUILD_PLAN.md` for the record.
...
- Undocumented-but-real scripts: `dev:all` (app + worker, D-073), `db:dev`, `db:seed`, `db:studio`, `archive:evidence`, `demo:walkthrough`, `demo:resonance`, `recompute:resonance`, `test:golden`, `test:mock-e2e`, and the `research:retired:m34:*` family behind `retired-guard.ts`.
