...
Migration 0024 (M62, D-141): `0024_m62_retire_geo_agent` drops the six `agent_*` tables from 0017 and their six agent-only enum types. `service_heartbeats` (also from 0017) is kept for the operator worker and now lives in `src/db/schema/ops.ts`. The `crypto_token` and `xai` labels stay in the shared enums because Postgres cannot drop an enum value in place.
...
| xAI (Grok) `xai` | Retired (D-141): the GEO agent and its registry entry were removed in M62; the label remains only in the `provider_id` DB enum | — | — | — | — | — | — | — | M36 |
