...
| `resonance_stimuli` (M17; M34A migration 0014) | `id`, `study_id`, `label`, `stimulus_kind`, `body_md`, `evidence_response_ids_json`, `framing_evidence_snapshot_id`, `position`, `created_at` | FK `study_id`; `stimulus_kind` in `measured_ai`, `corrected`, `repositioned`, `custom`; unique `(study_id, position)`; B2B `measured_ai` requires >=1 same-project response id (C-13); consumer `measured_ai` requires a same-project valid snapshot and byte-equal copied raw response (C-15); immutable after study approval |
| `framing_studies`, `framing_response_reviews`, `framing_annotations`, `framing_gap_classifications`, `framing_evidence_snapshots` (M34A, migration 0014) | Project/source-run workflow record; one review per denominator job; accepted/rejected literal offsets; manual gap decisions; copied JSON evidence payload + SHA-256 | Consumer projects only; fixed representation protocol pinned; codebook locks before reveal; all jobs remain in N including unavailable generations; accepted spans resolve uniquely against immutable raw text; snapshot repository is append-only and carries codebook/review/reveal/model/prompt/recurrence provenance into Simulation |
