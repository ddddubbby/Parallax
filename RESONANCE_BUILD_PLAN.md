...
audit_runs → jobs → responses          (unchanged machinery, all cost guards apply)
        ▼
extractions (schema_version='ssr-v1')  (SSR scoring instead of brand extraction:
...
2. **Migration `0008_resonance_studies.sql`** (additive-first, D-024 discipline):
   ...
   - `prompt_cells`: add `stimulus_id uuid null` FK, add `panel_persona_key text null`; **VERIFY first** whether `persona_id`/`market_id` are NOT NULL in migration 0000 (`grep -n "persona_id\|market_id" src/db/migrations/0000_init_full_schema.sql`) — if NOT NULL, `ALTER ... DROP NOT NULL` for both. Audit-path code never inserts nulls, so this loosening is safe for existing flows, but add the check in step 5's compile tests anyway.
...
3. **Core types & validation** — extend the Intent app-level type with literal `'simulation'` **only where cells are parsed/displayed**, NOT in `DEFAULT_INTENT_ALLOCATION` or any allocator/quota/frame table. Find every `Intent`-typed switch via `pnpm typecheck` after the type change and handle the new case explicitly (most should throw-or-skip with a comment "simulation cells never reach audit paths"). Zod schema for `panel_personas_json` entries: `{ key: string, label: string, age: number, incomeBand: string, location: string, behavioralProfile: string }` — the SSR-validated conditioning axes only (C-14; no gender/ethnicity fields — do not add them).
4. **Anchor fixtures** — `fixtures/ssr/anchor-sets.json`: `{ version: "purchase_intent.v1", construct: "purchase_intent", sets: [{ id: "set1", sentences: ["...1","...2","...3","...4","...5"] }, ...] }` with the 4 generic purchase-intent sets from the OSS skill (documented in the 2026-07-05 research session; short, generic, domain-independent; sentences[i] maps to Likert i+1). Loader `src/core/ssr-anchors.ts` validates shape (exactly 5 sentences per set, ≥4 sets) and exports `loadAnchorSets(version)`. **Anchors are C-4-frozen artifacts: editing sentences requires a NEW version string; the loader must refuse unknown versions loudly.** Unit tests.
...
6. **Run creation threading** — `createRun` currently takes a matrix version; VERIFY how it validates (read `src/modules/runner/actions.ts` + repo function). Resonance runs: allowed `run_mode` values `mock` and `live_validation`/`live_audit` with identical C-9 both-direction validation, k semantics unchanged (k=5 default; k=2 validation-labeled). Runs list/run detail pages must show a `SIM` badge when the run's matrix kind is resonance (join through matrix_versions). The run-creation UI for resonance lives on the study page ("Run this study" button) — reuse the existing run form component with the matrix preselected.
...
9. **Study builder UI** — `/projects/[id]/resonance`: studies list + create; study detail: panel-persona editor (validated fields only), stimuli editor with **evidence picker** (searchable list of the project's stored responses filtered to completed audit runs; selecting inserts the excerpt into `body_md` as a quoted block and records the response id into `evidence_response_ids_json`), approve button with cell-count preview, run button. Everything carries SimulatedBadge. Unconditioned studies (`conditioned=false`, operator must explicitly toggle) show a persistent "GENERIC — not evidence-conditioned" warning chip (C-13).
...
| `intent='simulation'` leaks into audit code paths | Frame filters (`metricIntentFilter`), allocator quotas, funnel heatmap would misbehave | Type-system sweep in step 3; audit recompute never sees resonance runs (M18 dispatch); heatmap queries are per-run |
| Migration drops NOT NULL that audit inserts rely on | None rely on it (they always set both) — but a sloppy future insert could | Compile tests assert audit-path cell creation still sets persona/market |
...
4. **SSR scoring service** — `src/modules/resonance/scoring.ts`:
   - `scoreResponse(responseId)`: creates the next-version `extractions` row per the existing versioning pattern (`extraction_version` increments; find the pattern in `src/modules/extraction/service.ts` and mirror it): `schema_version='ssr-v1'`, `extraction_model=<embedding model or 'mock-fixture'>`, `extracted_json={ kind:'ssr', anchorSetVersion, pmf:[..5], perSetPmfs, meanScore }`, `state='valid'` on success; embedding cost written to the extraction row's cost column; retry-once/dead-letter mirroring SM-2/SM-3 semantics.
...
6. **Resonance metrics recompute** — dispatch at the top of `recomputeMetrics(runId)` in `src/db/repositories/metrics.ts`: look up the run's matrix `kind`; `resonance` → `recomputeResonanceMetrics(runId)`, else the existing audit path untouched. Resonance recompute (same delete-then-rebuild idempotent shape):
   - Eligibility: D-014 analog — latest extraction is `valid` (or qa_reviewed) `ssr-v1` row; refusal concept N/A in v1.
...
- DB-backed resonance e2e: M17's study → mock run → assert every response has exactly one `ssr-v1` extraction with PMF summing to 1, metrics rows exist for every scope, recompute twice → byte-identical (C-5).
