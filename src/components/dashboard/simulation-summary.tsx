      ...
      {summaries.length === 0 ? (
        <div className="rounded-xl border border-ink/15 bg-paper-2/30 p-6">
          ...
          <p className="mt-1 font-mono text-xs text-ink/45">
            approve a study under Setup → Simulation studies, then run it
          </p>
          <Link
            href={`/projects/${projectId}/resonance`}
            className="label-mono mt-3 inline-block text-xs text-accent-ink hover:text-accent"
          >
            Simulation studies →
          </Link>
        </div>
          ...
          {summaries.map((summary) => (
            ...
            >
              ...
              {summary.engines.length === 0 ? (
                <p className="font-mono text-xs text-ink/45">Approved study — no completed run yet</p>
              ) : (
