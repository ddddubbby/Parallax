...
export interface MockResonanceFixture {
  id: string;
  text: string;
}
...
const RESONANCE_FIXTURES_PATH = join(process.cwd(), "fixtures", "mock-responses", "resonance-fixtures.json");

let cached: MockFixture[] | null = null;
let resonanceCached: MockResonanceFixture[] | null = null;
...
export function loadMockResonanceFixtures(): MockResonanceFixture[] {
  if (!resonanceCached) {
    resonanceCached = JSON.parse(readFileSync(RESONANCE_FIXTURES_PATH, "utf8")) as MockResonanceFixture[];
  }
  return resonanceCached;
}
